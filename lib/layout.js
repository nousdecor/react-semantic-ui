      ...
      render: function() {
        ...
        if (totalPages && totalPages > 1) {
          ...
          children = children.map(function(child, i) {
            if (child === current) {
              return React.DOM.div({className: 'active item', key: i}, child);
            } else {
              return React.DOM.a({className: 'item', href: '#' + child, key: i, onClick: common.eventBinder(child, 'onChange', self, true)}, child);
            }
          });
